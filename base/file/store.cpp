 ...
 */
#define DB_STRINGKEY                                                           \
  0 /** 4 <= max_key_size <= 128, include '\0', 包含'\0'在内 */
...
typedef struct {
  off_t self; /** 数据块位置 */
  size_t
      num; /** 当前数据块作为btree_key时，表示btree节点的关键字数；当前数据块作为btree_value时，表示块的引用数 */
  off_t free;        /** 空闲链表节点 */
  uint32_t use : 1;  /** 当前数据块是否被使用 */
  uint32_t type : 1; /** 当前数据块作为btree_key或btree_value */
  uint32_t
      leaf : 1; /** 当前数据块作为btree_key时，表示节点为叶子节点或非叶子节点 */
  uint32_t last : 29; /** 当前数据块作为btree_value时，表示数据块未分配的空间 */
} btree_node;

#define btree_key_ptr(db, node, n)                                             \
  ((btree_key *)((char *)(node) + sizeof(*node) + (db->key_align) * (n)))
...
static int cmp_bytes(void *a, void *b, size_t n) { return memcmp(a, b, n); }
...
inline static int node_create(db_t *db, btree_node *node, int leaf, int type) {
  ...
  memset((char *)node + sizeof(btree_node), 0,
         DB_BLOCK_SIZE - sizeof(btree_node));
...
int db_create(const char *path, KEY_TYPE key_type, size_t max_key_size) {
  ...
  char *buf = (char *)calloc(DB_HEAD_SIZE + DB_BLOCK_SIZE, sizeof(char));
  if (buf == NULL) {
    close(fd);
    return -1;
  }
...
int db_checker(db_t *db) {
  ...
  if (stat.st_size < DB_HEAD_SIZE + DB_BLOCK_SIZE ||
      (stat.st_size - DB_HEAD_SIZE) % DB_BLOCK_SIZE != 0) {
    return -1;
  }
  ...
  if (db->key_align !=
      db_align(sizeof(btree_key) + db->key_size, DB_ALIGNMENT)) {
    return -1;
  }
  ...
  if (key_total != value_total || key_total != db->key_total ||
...
int db_open(db_t **db, const char *path) {
  ...
  *db = (db_t *)malloc(DB_HEAD_SIZE + DB_BLOCK_SIZE * 5);
  if (*db == NULL) {
    close(fd);
    return -1;
  }
...
#define keycpy(db, dest, src, n)                                               \
  memmove(dest, src, (db)->key_align *((n) + 1)); // 需要包括 src[n]->child
 ...
 */
inline static void btree_split_child(db_t *db, btree_node *node, int position,
                                     btree_node *sub_x, btree_node *sub_y) {
  size_t n = ceil(db->M);

  keycpy(db, btree_key_ptr(db, sub_y, 0), btree_key_ptr(db, sub_x, n + 1),
         sub_x->num - n - 1);
  sub_y->num = sub_x->num - n - 1;
  sub_x->num = n;

  keycpy(db, btree_key_ptr(db, node, position + 1),
         btree_key_ptr(db, node, position), node->num - position);
  memcpy(btree_key_ptr(db, node, position), btree_key_ptr(db, sub_x, n),
         db->key_align);
 ...
 * @param sub_y node->child[position+1] = sub_y
 */
inline static int btree_merge(db_t *db, btree_node *node, int position,
                              btree_node *sub_x, btree_node *sub_y) {
  memcpy(btree_key_ptr(db, sub_x, sub_x->num)->key,
         btree_key_ptr(db, node, position)->key,
         db->key_align - sizeof(btree_key));
  btree_key_ptr(db, sub_x, sub_x->num)->value =
      btree_key_ptr(db, node, position)->value;

  keycpy(db, btree_key_ptr(db, sub_x, sub_x->num + 1),
         btree_key_ptr(db, sub_y, 0), sub_y->num);
  sub_x->num += (1 + sub_y->num);

  keycpy(db, btree_key_ptr(db, node, position),
         btree_key_ptr(db, node, position + 1), node->num - position - 1);
  btree_key_ptr(db, node, position)->child = sub_x->self;
  node->num--;
  ...
  if (node->num != 0) {
    ...
    memcpy((char *)node + sizeof(btree_node),
           (char *)sub_x + sizeof(btree_node),
           DB_BLOCK_SIZE - sizeof(btree_node));
...
int db_insert(db_t *db, void *key, void *value, size_t value_size) {
  ...
  if (sizeof(btree_node) +
          db_align(sizeof(btree_value) + value_size, DB_ALIGNMENT) >
      DB_BLOCK_SIZE) {
    errno = E2BIG;
    return -1;
  }
  ...
  btree_node *node =
      (btree_node *)((char *)db + DB_HEAD_SIZE + DB_BLOCK_SIZE * 0);
  btree_node *sub_x =
      (btree_node *)((char *)db + DB_HEAD_SIZE + DB_BLOCK_SIZE * 1);
  btree_node *sub_y =
      (btree_node *)((char *)db + DB_HEAD_SIZE + DB_BLOCK_SIZE * 2);
  btree_node *valnode =
      (btree_node *)((char *)db + DB_HEAD_SIZE + DB_BLOCK_SIZE * 3);
  ...
  if (node->num >= db->M - 1) {
    ...
    if (node_create(db, sub_x, node->leaf, TYPE_KEY) == -1 ||
        node_create(db, sub_y, node->leaf, TYPE_KEY) == -1) {
      return -1;
    }

    sub_x->num = node->num;
    memcpy((char *)sub_x + sizeof(btree_node),
           (char *)node + sizeof(btree_node),
           DB_BLOCK_SIZE - sizeof(btree_node));
  ...
  keycpy(db, btree_key_ptr(db, node, i + 1), btree_key_ptr(db, node, i),
         node->num - i);
  switch (db->key_type) {
  case DB_STRINGKEY:
    strcpy((char *)(btree_key_ptr(db, node, i)->key), (char *)key);
    break;
  case DB_BYTESKEY:
  case DB_INT32KEY:
  case DB_INT64KEY:
  ...
  btree_key_ptr(db, node, i)->value =
      valnode->self + last; // 记录value所在数据块的位置 + 偏移
  node->num++;
  node_flush(db, node);
  db->key_total++;
  head_flush(db);
...
#define MORE 2
  ...
  btree_node *sub_y =
      (btree_node *)((char *)db + DB_HEAD_SIZE + DB_BLOCK_SIZE * 3);
  btree_node *sub_w =
      (btree_node *)((char *)db + DB_HEAD_SIZE + DB_BLOCK_SIZE * 4);
  ...
  while (node->leaf == BTREE_NON_LEAF) {
    ...
    // 在非叶子节点中匹配到，需要找到在前缀或后缀关键字（该关键字只会在叶子结点中）
    if (i >= 0) {
    ...
    // 自上而下调整子树，保证最后在叶子节点处方便删除关键字（自上而下，确保子树关键字个数大于ceil(M)）
    node_seek(db, sub_x, btree_key_ptr(db, node, i)->child);
    ...
    if (i + 1 <= node->num && sub_y->num > ceil(db->M)) {
      ...
      memcpy(btree_key_ptr(db, sub_x, sub_x->num)->key,
             btree_key_ptr(db, node, i)->key,
             db->key_align - sizeof(btree_key));
      btree_key_ptr(db, sub_x, sub_x->num)->value =
          btree_key_ptr(db, node, i)->value;
      btree_key_ptr(db, sub_x, sub_x->num + 1)->child =
          btree_key_ptr(db, sub_y, 0)->child;
      sub_x->num++;

      memcpy(btree_key_ptr(db, node, i)->key, btree_key_ptr(db, sub_y, 0)->key,
             db->key_align - sizeof(btree_key));
      btree_key_ptr(db, node, i)->value = btree_key_ptr(db, sub_y, 0)->value;
      keycpy(db, btree_key_ptr(db, sub_y, 0), btree_key_ptr(db, sub_y, 1),
             sub_y->num - 1);
      sub_y->num--;
      ...
      keycpy(db, btree_key_ptr(db, sub_x, 1), btree_key_ptr(db, sub_x, 0),
             sub_x->num);
      memcpy(btree_key_ptr(db, sub_x, 0)->key,
             btree_key_ptr(db, node, i - 1)->key,
             db->key_align - sizeof(btree_key));
      btree_key_ptr(db, sub_x, 0)->value =
          btree_key_ptr(db, node, i - 1)->value;
      btree_key_ptr(db, sub_x, 0)->child =
          btree_key_ptr(db, sub_w, sub_w->num)->child;
      sub_x->num++;

      memcpy(btree_key_ptr(db, node, i - 1)->key,
             btree_key_ptr(db, sub_w, sub_w->num - 1)->key,
             db->key_align - sizeof(btree_key));
      btree_key_ptr(db, node, i - 1)->value =
          btree_key_ptr(db, sub_w, sub_w->num - 1)->value;
      sub_w->num--;
  ...
  if (flag == LESS) {
    ...
    memcpy(btree_key_ptr(db, node_match, i_match)->key,
           btree_key_ptr(db, node, 0)->key, db->key_align - sizeof(btree_key));
    btree_key_ptr(db, node_match, i_match)->value =
        btree_key_ptr(db, node, 0)->value;
    keycpy(db, btree_key_ptr(db, node, 0), btree_key_ptr(db, node, 1),
           node->num - 1);
    node->num--;
    ...
    memcpy(btree_key_ptr(db, node_match, i_match)->key,
           btree_key_ptr(db, node, node->num - 1)->key,
           db->key_align - sizeof(btree_key));
    btree_key_ptr(db, node_match, i_match)->value =
        btree_key_ptr(db, node, node->num - 1)->value;
    node->num--;
    node_flush(db, node_match);
    node_flush(db, node);
  } else {
    ...
    keycpy(db, btree_key_ptr(db, node, i), btree_key_ptr(db, node, i + 1),
           node->num - i - 1);
    node->num--;
    node_flush(db, node);
  }
  ...
  node_seek(db, node,
            DB_HEAD_SIZE + ((offset - DB_HEAD_SIZE) & ~(DB_BLOCK_SIZE - 1)));
  node->num--;
